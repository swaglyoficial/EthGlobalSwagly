...
import { attestActivityCompletion } from '@/lib/attestations-service'
...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      ...
      // ========================================
      console.log('====================================')
      console.log('📝 CREANDO ATTESTATION ON-CHAIN')
      console.log('====================================')
      ...
      try {
        const attestation = await attestActivityCompletion({
          ...
          scanType: 'nfc', // Determinar si es NFC o QR basado en el scan
          activityName: nfc.activity.name
        })

        console.log('✅ Attestation creada:')
        console.log('  UID:', attestation.uid)
        console.log('  TX Hash:', attestation.txHash)
        console.log('  Explorer:', `https://sepolia.scrollscan.com/tx/${attestation.txHash}`)
        console.log('====================================')

        attestationData = {
          uid: attestation.uid,
          txHash: attestation.txHash,
          explorerUrl: `https://sepolia.scrollscan.com/tx/${attestation.txHash}`
        }
      } catch (attestError) {
        // Si falla la attestation, solo logueamos el error pero no fallamos todo el flujo
        console.error('⚠️ Error al crear attestation (no crítico):', attestError)
        console.log('El scan y los tokens se procesaron correctamente')
      }

      // Respuesta exitosa: scan registrado, tokens enviados y attestation creada
      return NextResponse.json({
        success: true,
        ...
        attestation: attestationData, // Datos de la attestation on-chain
        scanData: {
          walletAddress,
          tokens: nfc.activity.numOfTokens,
          activityId: nfc.activityId,
          ...
          attestationTxHash: attestationData?.txHash,
        },
      })
    } catch (claimError) {
